...
// fungsi getrear digunakan untuk mendapatkan nilai paling belakang pada deque.
void getrear()
{
	if((f==-1) && (r==-1))
	{
...
// fungsi delete_front() untuk menghapus elemen dari depan
void delete_front()
{
	if((f == - 1) && (r == - 1))
	{
	...
	{
		printf("\Elemen yg dihapus adalah %d", deque[f]);
		f = -1;
		r = -1;
	}
	else if(f == (ukuran - 1))
...
// fungsi delete_rear() digunakan untuk menghapus elemen dari belakang
void delete_rear()
{
	if((f == -1) && (r == -1))
	{
	...
	{
		printf("\Elemen yg dihapus adalah %d", deque[r]);
		f == -1;
		r == -1;
	}
	else if(r == 0)
	{
		printf("\nElemen yg dihapus adalah %d", deque[r]);
		r = ukuran - 1;
	}
	else
	{
